...
parser.add_argument("--teleop_device", type=str, default="keyboard", choices=['keyboard', 'so101leader', 'bi-so101leader', 'omx_keyboard'], help="Device for interacting with environment")
...
def main():  # noqa: C901
    ...
    # create controller
    if args_cli.teleop_device in ["keyboard", "omx_keyboard"]:
        teleop_interface = Se3Keyboard(env, sensitivity=2.0 * args_cli.sensitivity)
        ...
        raise ValueError(
            f"Invalid device interface '{args_cli.teleop_device}'. Supported: 'keyboard', 'so101leader', 'bi-so101leader', 'omx_keyboard'."
        )
